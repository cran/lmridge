cv.plot <- function(x, abline = TRUE,...) {
  ...
  plot(x$K, kest(x)$GCV, main = "GCV vs K", xlab = "K", ylab = "GCV", lwd = 2, type = 'l' )
  if (abline) {
    abline(v = K, lty = 2)
    ...
    text(K, max(kest(x)$GCV), paste(c("min GCV=", " at K="), c(round(min(kest(x)$GCV), 3), K), collapse=" "), col="red", pos=4)
  }

  plot(
    x$K, kest(x)$CV, main = "CV vs K", xlab = "K", ylab = "CV", lwd = 2, type ='l')
  if (abline) {
    abline(v = K, lty = 2)
...
#    text(K, min(kest(x)$CV), paste("CV=", round(min(kest(x)$CV),3)), col="blue", pos=4)
    text(K, max(kest(x)$CV), paste(c("min CV=", " at K="), c(round(min(kest(x)$CV), 3), K), collapse=" "), col="red", pos=4)

  }
  mtext(
    "Cross Validation Plots", outer = TRUE, side = 3, cex = 1.8, line = 1
  )
